...
export default function App() {
  ...
  const handleSearch = (e) => {
    ...
    const filteredFiles = myFiles.filter(
      (file) => file.name.toLowerCase().includes(keyword.toLowerCase())
    );
  ...
  useEffect(() => {
    const isFileHidden = localStorage.getItem('fileHidden');
    if (isFileHidden) {
      setFileHidden(true);
    }
  }, []);
  ...
  useEffect(() => {
    if (fileHidden) {
      localStorage.setItem('fileHidden', 'true');
    } else {
      localStorage.removeItem('fileHidden');
    }
  }, [fileHidden]);
        ...
        <div style={styles.container}>
          ...
          <div style={styles.controlTools}>
            ...
            <button style={styles.controlButton} onClick={() => {
              const sortedFiles = [...myFiles].sort((a, b) => a.name.localeCompare(b.name));
              setMyFiles(sortedFiles);
            }}>
              Sort
            </button>
          ...
          <div style={styles.fileContainer}>
            <div style={{ width: "100%", padding: 10 }}>
              {(searchKeyword ? searchedFiles : myFiles).map((file) => {
                if (
